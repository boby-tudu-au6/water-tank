...
export const createMatrix = (list) => {
  const arr = []
  const height = [...list].sort((a, b) => b - a)[0]
  for (let i = height; i > 0; i--) {
      const items = []
      list.forEach((item, position) => {
          if (item >= i) items.push(1)
          else items.push(0)
      })
      arr.push(items)
  }
...
export const fillWater = (data) => {
  ...
  for (let i = 0; i < newMatrix.length; i++) {
      ...
      for (let j = 0; j < newMatrix[i].length; j++) {
          if (start === true && newMatrix[i][j] === 1) {
              start = false
              tempPair.push([startPos, j])
          }
          if (newMatrix[i][j] === 1 && newMatrix[i][j + 1] === 0) {
              start = true
              startPos = j
          }
          if (j === newMatrix[i].length - 1 && start === true) tempPair.push([])
      }
      pairs.push(tempPair)
      startPos = 0
      start = false
  ...
  for (let a = 0; a < pairs.length; a++) {
      for (let j = 0; j < pairs[a].length; j++) {
          for (let i = pairs[a][j][0] + 1; i < pairs[a][j][1]; i++) {
              newMatrix[a][i] = 2
          }
      }
  }
